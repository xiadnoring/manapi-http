...
UTEST(compress, compress_zstd) {
    auto ctx = init_ctx(utest_result);
    {
        ...
        {
            manapi::compress::zstd_compress dc (1, 0);
            ri = dc.compress(z, false).unwrap();
            ri.push_back(dc.compress( manapi::slice () , true).unwrap());
        }

        {
            manapi::compress::zstd_decompress dd;
            ro = dd.decompress(ri, false).unwrap();
            ro.push_back(dd.decompress(manapi::slice (), true).unwrap());
        }
...
UTEST(compress, compress_zstd_small_data) {
    auto ctx = init_ctx(utest_result);
    {
        assert(sizeof (MANAPI__COMPRESS_TEXT) == 50);
        auto st = std::string (MANAPI__COMPRESS_TEXT);
...
UTEST(compress, compress_brotli) {
    auto ctx = init_ctx(utest_result);
    {
        manapi::slice z;
        for (int i = 0; i < 2; i++) {
        ...
        {
            manapi::compress::brotli_compress dc (11, 22, 0);
            ri = dc.compress(z, false).unwrap();
            ri.push_back(dc.compress( manapi::slice () , true).unwrap());
        }

        {
            manapi::compress::brotli_decompress dd;
            ro = dd.decompress(ri, false).unwrap();
            ro.push_back(dd.decompress(manapi::slice (), true).unwrap());
        }
...
UTEST(compress, compress_brotli_small_data) {
    auto ctx = init_ctx(utest_result);
    {
        assert(sizeof (MANAPI__COMPRESS_TEXT) == 50);
        auto st = std::string (MANAPI__COMPRESS_TEXT);
