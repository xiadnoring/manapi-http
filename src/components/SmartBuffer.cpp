...
manapi::net::worker::smart_w_buffer::smart_w_buffer(const std::function<ssize_t (void *, ssize_t size, bool flag)> &callback, size_t sent, ssize_t frame_size) {
    this->flag = false;
    this->sent = sent;
...
manapi::net::worker::smart_w_buffer & manapi::net::worker::smart_w_buffer::operator=(smart_w_buffer &&n) noexcept {
    ...
    this->sent = static_cast<size_t>(n.sent);
...
ssize_t manapi::net::worker::smart_w_buffer::_work(bool flag) {
    ...
    if (flag && (ptr < end)) {
        ...
        auto rhs = callback (ptr, end - ptr, flag);
        ptr += rhs;
    }
...
manapi::net::worker::smart_r_buffer::smart_r_buffer(const std::function<void(int)> &callback, int frame_size) {
    this->frame_size = frame_size;
    this->callback = callback;
...
ssize_t manapi::net::worker::smart_r_buffer::read(void *c, size_t len) {
