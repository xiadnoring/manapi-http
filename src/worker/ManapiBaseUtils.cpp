...
int manapi::net::worker::prepared::event_flags(const shared_conn &conn) MANAPIHTTP_NOEXPECT {
    return event_flags(conn, conn->as<connection_prepared_base_t>());
}

int manapi::net::worker::prepared::event_flags(const shared_conn &conn, connection_prepared_base_t *data) MANAPIHTTP_NOEXPECT {
    return (conn->as<connection_prepared_base_t>()->flags) & base::CONN_MASK_GETTING;
}

manapi::bytebuffer manapi::net::worker::prepared::recv_first_buffer(const shared_conn &conn) MANAPIHTTP_NOEXPECT {
    auto const data = conn->as<connection_prepared_t>();
    return recv_first_buffer(conn, data);
}

manapi::bytebuffer manapi::net::worker::prepared::recv_first_buffer(const shared_conn &conn, connection_prepared_t *data) MANAPIHTTP_NOEXPECT {
...
void manapi::net::worker::prepared::waiting(const shared_conn &conn, connection_prepared_base_t *data, bool state) MANAPIHTTP_NOEXPECT {
...
void manapi::net::worker::prepared::waiting(const shared_conn &conn, bool state) MANAPIHTTP_NOEXPECT {
    auto const data = conn->as<connection_prepared_base_t>();
    return waiting(conn, data, state);
}

void manapi::net::worker::prepared::flush_read_(worker::base *w, const shared_conn &conn,connection_prepared_t *data) MANAPIHTTP_NOEXPECT {
...
bool manapi::net::worker::prepared::is_writable(http::config *config, const shared_conn &conn,connection_prepared_t *data) MANAPIHTTP_NOEXPECT {
    return data->top->send_size <= config->max_buffer_stack
        && data->transfered < config->speed_limit_rate;
}

std::size_t manapi::net::worker::prepared::recv_count(const shared_conn &conn) MANAPIHTTP_NOEXPECT {
    auto const s = conn->as<connection_prepared_t>();
    return recv_count(conn, s);
}

std::size_t manapi::net::worker::prepared::recv_count(const shared_conn &conn, connection_prepared_t *data) MANAPIHTTP_NOEXPECT {
    return data->top->recv_size;
}

void manapi::net::worker::prepared::top_buffer_clear(connection_prepared_t *s) MANAPIHTTP_NOEXPECT {
    if (s->top) {
        if (s->top->recv_size) {
...
void manapi::net::worker::prepared::event_callback_clear(const shared_conn &conn,connection_prepared_base_t *s) MANAPIHTTP_NOEXPECT {
    if (s->ev_callback) {
...
void manapi::net::worker::prepared::timer_clear(manapi::timer t) MANAPIHTTP_NOEXPECT {
    if (t) {
        t.stop();
        t.clear();
    }
...
void manapi::net::worker::prepared::feed_event(worker::base *w, const shared_conn &conn, connection_prepared_t *data, int flags, const char *buff, ssize_t size, ibuffpool_t *p) MANAPIHTTP_NOEXPECT {
    if (flags & ev::READ) {
...
void manapi::net::worker::prepared::feed_event(worker::base *w, const shared_conn &conn, int flags,const char *buff, ssize_t size, ibuffpool_t *p) MANAPIHTTP_NOEXPECT {
    auto const data = conn->as<connection_prepared_t>();
    return feed_event(w, conn, data, flags, buff, size, p);
}

void manapi::net::worker::prepared::update_limit_rate_connection(const shared_conn &sconn, connection_prepared_base_t *data, worker::base *w, http::config *config, wrk_interface_global_t *global) MANAPIHTTP_NOEXPECT {
...
void manapi::net::worker::prepared::update_limit_rate_connection(const shared_conn &sconn, worker::base *w, http::config *config, wrk_interface_global_t *global) MANAPIHTTP_NOEXPECT {
    update_limit_rate_connection(sconn, sconn->as<connection_prepared_base_t>(), w, config, global);
}

std::unique_ptr<manapi::net::worker::worker_watcher_cb> manapi::net::worker::prepared::event_on(const shared_conn &conn,std::unique_ptr<worker_watcher_cb> callback) MANAPIHTTP_NOEXPECT {
