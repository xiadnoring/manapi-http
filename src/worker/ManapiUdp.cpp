...
void manapi::net::worker::udp::init() {
...

    {
        ...
        if (auto rhs = this->udp_accept_->s_bind(reinterpret_cast<sockaddr *>(&this->sockaddrin), bind_flags)) {
            manapi::async::current()->logger()->error(manapi::logger::default_service, ERR_AI_FAILED_PRECONDITION, "couldn't bind socket due to result - {}", rhs);
            goto err;
        }
    }
    ...
    return;
err:
    THROW_MANAPIHTTP_EXCEPTION2 (ERR_AI_FAILED_PRECONDITION, "couldn't initialize udp connection");
}
