...
manapi::net::worker::wrk_interface_global_t * manapi::net::worker::http_v3::wrk_global() MANAPIHTTP_NOEXCEPT {
    return this->w->wrk_global();
}

void manapi::net::worker::http_v3::wrk_global(wrk_interface_global_t *data) MANAPIHTTP_NOEXCEPT {
    this->w->wrk_global(data);
}

manapi::net::http::config * manapi::net::worker::http_v3::config() MANAPIHTTP_NOEXCEPT {
...
void manapi::net::worker::http_v3::close_connection(shared_conn conn, int flags) MANAPIHTTP_NOEXCEPT {
    ...
    manapi_log_trace(debug::LOG_TRACE_MEDIUM, "http3:close_connection() %p id=%zu flags=%d",
        data, this->w->stream_id(conn), flags);
        ...
        if (data->ev_callback) {
            if (data->flags & CONN_CLOSED) {
                ...
                if (flags & ev::READ) {
                    ...
                    if (data->flags & CONN_RECV_END) {
                        if (manapi::net::worker::http_v3::call_user_callback(&data->ev_callback, conn,
...
ssize_t manapi::net::worker::http_v3::sync_write_ex(const shared_conn &conn, ev::buff_t *buff, uint32_t nbuff, ssize_t size, bool finish, int maxcnt) MANAPIHTTP_NOEXCEPT {
