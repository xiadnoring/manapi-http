...
manapi::future<CURLcode> manapi::net::fetch::async_curl_perform() {
    ...
    co_return co_await async::promise<CURLcode> (this->site.taskpool, [&attempts, &w, this, &fds] (const std::function<void(CURLcode v)> &resolve, const std::function<void(std::exception_ptr)> &reject) -> future<> {
        ...
        auto reject_cb = [this, &fds, &w, reject] () mutable -> void {
            ...
            /* error */
            for (auto &w_it: fds) {
                this->site.stop_watcher_fd(*w_it.second);
            }
            fds.clear();
            ...
            if (running_handles) {
                ...
                for (int i = 0; i <= maxfd; ++i) {
                    if (FD_ISSET(i, &fd_read)) {
                        auto p = (i << 1) | 0x01;
                        if (!fds.contains(p)) {
                            auto wr = this->site.create_watcher_fd(i, ev::READ, [this, &shared_w, &fds, p] (ev::io &wr, int revents) -> void {
                                shared_w->send();

                                fds.erase(p);
                                this->site.stop_watcher_fd(wr);
                            });
                            wr->start();
                    ...
                    if (FD_ISSET(i, &fd_write)) {
                        auto p = (i << 1);
                        if (!fds.contains(p)) {
                            auto ww = this->site.create_watcher_fd(i, ev::WRITE, [this, &shared_w, &fds, p] (ev::io &ww, int revents) -> void {
                                shared_w->send();

                                fds.erase(p);
                                this->site.stop_watcher_fd(ww);
                            });
                            ww->start();
