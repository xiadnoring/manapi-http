...
manapi::status_or<std::string> manapi::crypto::aead_decrypt(std::string_view cipher_password, std::string_view aad, std::string_view key, std::string_view iv, std::string_view tag, ciphers algorithm) {
    ...
    if (!n)
        return status_invalid_argument("EVP_CIPHER_CTX_new() failure");

    auto ctx = n.get();

    if (!EVP_DecryptInit_ex(ctx, cipher, nullptr, nullptr, nullptr))
        return status_invalid_argument("EVP_DecryptInit_ex() failure");
    ...
    if (!EVP_DecryptUpdate(ctx, nullptr, &len, reinterpret_cast<const uint8_t *>(aad.data()), static_cast<int>(aad.size())))
        return status_invalid_argument("EVP_DecryptUpdate() failure");
    ...
    if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, taglen, (void*)(tag.data()) ))
        return status_invalid_argument("EVP_CIPHER_CTX_ctrl() failure");
    ...
    if (!ctx)
        return status_invalid_argument ("wolfSSL_EVP_CIPHER_CTX_new() failure");

    if (!wolfSSL_EVP_DecryptInit_ex(ctx, cipher, nullptr, nullptr, nullptr))
        return status_invalid_argument ("wolfSSL_EVP_DecryptInit_ex() failure");
    ...
    if (!wolfSSL_EVP_DecryptUpdate(ctx, nullptr, &len, reinterpret_cast<const uint8_t *>(aad.data()), static_cast<int>(aad.size())))
        return status_invalid_argument ("wolfSSL_EVP_DecryptUpdate() failure");
    ...
    if (!wolfSSL_EVP_CIPHER_CTX_ctrl(ctx, WOLFSSL_EVP_CTRL_GCM_SET_TAG, taglen, (void*)(tag.data()) ))
        return status_invalid_argument ("wolfSSL_EVP_CIPHER_CTX_ctrl() failure");
    ...
    if (1 != rhs)
        return status_invalid_argument ("wolfSSL_EVP_DecryptFinal_ex() failure");
...
manapi::status_or<std::string> manapi::crypto::aead_encrypt(std::string_view data, std::string_view aad, std::string_view key, std::string_view iv, std::string &tag, ciphers algorithm) {
    std::string plaintext;
#if MANAPIHTTP_CRYPTO_LIBRARY == 1
    ...
    if (!EVP_EncryptInit_ex(ctx, cipher, nullptr, nullptr, nullptr))
        return status_invalid_argument("EVP_EncryptInit_ex() failure");
    ...
    if (!EVP_EncryptUpdate(ctx, nullptr, &len, reinterpret_cast<const uint8_t *>(aad.data()), static_cast<int>(aad.size())))
        return status_invalid_argument("EVP_EncryptUpdate() failure");
    ...
    if (!wolfSSL_EVP_EncryptInit_ex(ctx, cipher, nullptr, nullptr, nullptr))
        return status_invalid_argument("wolfSSL_EVP_EncryptInit_ex() failure");
    ...
    if (!wolfSSL_EVP_EncryptUpdate(ctx, nullptr, &len, reinterpret_cast<const uint8_t *>(aad.data()), static_cast<int>(aad.size())))
        return status_invalid_argument("wolfSSL_EVP_EncryptUpdate() failure");
    ...
    if (1 != rhs)
        return status_invalid_argument("wolfSSL_EVP_EncryptFinal_ex() failure");
