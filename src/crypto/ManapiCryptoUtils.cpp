...
manapi::future<void> manapi::crypto::async_random_string(std::shared_ptr<async::context> ctx, char *buff, size_t len, async::cancellation_action cancellation) {
        ...
        co_await ctx->eventloop()->custom_callback([reject = std::move(reject), buff, len, resolve = std::move(resolve), cancellation] (event_loop *ev) mutable
            ...
            try {
                ...
                if (cancellation.contains_cancel_callback()) {
                    cancellation.cancel_callback([w, resolve = std::move(resolve)] (std::shared_ptr<manapi::async::context> ctx) mutable
                        -> void {
