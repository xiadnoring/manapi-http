...
#endif
std::string manapi::crypto::hmac_digest(std::string_view key, std::string_view data, hashes hash_algorithm) {
#if MANAPIHTTP_OPENSSL_DEPENDENCY
...
std::string manapi::crypto::hkdf_extract(std::string_view salt, std::string_view ikm, hashes hash) {
    if (salt.empty()) {
...
std::string manapi::crypto::hkdf_expand(std::string_view prk, std::string_view info, int length, hashes algorithm) {
    std::string t;
    std::string okm;
    int i = 0;
    std::string ninfo;
...
std::string manapi::crypto::hkdf(std::string_view salt, std::string_view ikm, std::string_view info, int length, hashes hash) {
