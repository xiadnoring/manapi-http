...
manapi::json::json(STRING_VIEW str, const bool &parse) {
    if (parse)
    {
        this->_parse(str);
    }
...
manapi::json &manapi::json::operator=(const BOOLEAN &b) {
    _set_bool(b);
...
void manapi::json::insert(const STRING &key, manapi::json obj) {
    ...
    as_object().insert({key, std::move(obj)});
}
