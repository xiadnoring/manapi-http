...
namespace manapi {
    ...
    private:
        static json_error::status_or<const manapi::json *> next_parent_cb (json_builder *p);
        void _reset ();
        json_error::status _parse (std::string_view plain_text, size_t &j, bool root = true);
        json_error::status _check_type (std::string_view plain_text, size_t &j);
        json_error::status _build_string (std::string_view plain_text, size_t &j);
        json_error::status _build_numeric (std::string_view plain_text, size_t &j);
        json_error::status _build_numeric_string (std::string_view plain_text, size_t &j);
        json_error::status _build_object (std::string_view plain_text, size_t &j);
        json_error::status _build_array (std::string_view plain_text, size_t &j);
        json_error::status _check_end (std::string_view plain_text, size_t &j);

        void _reset_type ();
        bool _next_type ();
        bool _next_parent ();
        bool _check_eq_type ();
        manapi::json_error::status _check_max_mean (bool building = false);
        manapi::json_error::status _check_min_mean ();
        manapi::json_error::status _check_type_none_complex_value ();
        manapi::json_error::status _check_default ();
        manapi::json_error::status _check_meta_value ();
        manapi::json_error::status _check_string ();
        manapi::json_error::status _check_numeric ();
        manapi::json_error::status _check_object ();
        manapi::json_error::status _check_array ();
