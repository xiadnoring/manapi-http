...
namespace manapi {
    ...
    public:
        ...
        manapi::future<> edit (const std::shared_ptr<worker_t> &m, std::move_only_function<bool(manapi::json &data)> cb);

        manapi::future<> edit_async (const std::shared_ptr<worker_t> &m, std::move_only_function<manapi::future<bool>(manapi::json &data)> cb);
        ...
        T *as () { return static_cast<T *>(this->pointer()); }
    private:
        void notify_ (const std::shared_ptr<worker_t> &m);
