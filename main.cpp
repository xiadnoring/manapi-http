...
int main (int argc, const char *argv[]) {
    ...
    auto const ctx = manapi::async::context::create(4);
    ctx->eventloop()->setup_handle_interrupt();
    ctx->logger()->callback(
         [mx = std::make_shared<manapi::async::tmutex> ()](manapi::logger_type type, std::string_view service, int error_code, std::string msg) -> void {
         manapi::async::run(manapi::async::invoke(+[](std::shared_ptr<manapi::async::tmutex> mx, manapi::logger_type type, std::string_view service, int error_code, std::string msg) -> manapi::future<> {
             ...
             auto &s = (type == manapi::logger_type::LOGGER_ERROR) ? std::cerr : std::cout;
             s << "[" << service.substr(1) << "][" << error_code << "]: " << msg << "\n";
    ...
    ctx->run(ctx, 0, [&] (auto cb) -> void {
        ...
        auto db = std::make_shared<manapi::ext::pq::connection>();
        http router (server_ctx);

        router.GET ("/", [db] (http::req &req, http::resp &resp)
            -> manapi::future<> {
        ...
        router.GET("/pq/[id]",
            [db, mx = manapi::async::mutex()](manapi::net::http::request& req, manapi::net::http::response& resp) mutable
             -> manapi::future<> {
             auto lk = co_await mx.lock_guard();
             try {
                 auto res1 = co_await db->exec("INSERT INTO "
                 "for_test (id, str_col) VALUES ($2, $1);","no way",
                     std::stoll(req.param("id")));
             }
             catch (...) {
                 /* already exists */
             }
        ...
        manapi::async::run ([router, db] () mutable
            -> manapi::future<> {
