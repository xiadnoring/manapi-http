...
//                });
            ...
            router->GET("/hashfile", [] (http::req &req, http::resp &resp) -> manapi::future<> {
                manapi::hash::sha256 sha;
                auto z = manapi::unwrap(co_await manapi::fs::async_open("/home/timur/Загрузки/VideoDownloader/ufa.mp4", manapi::ev::FS_O_RDONLY, 0755));
                manapi::unwrap (co_await manapi::hash::hash_file( &sha, z.get() ));
                std::string g;
                g.resize(sha.final_size());
                sha.final(reinterpret_cast<uint8_t  *>(g.data()));
                g = manapi::crypto::strdec2strhex(g).unwrap();
                co_return resp.text(std::move(g)).unwrap();
            }).unwrap();
            ...
            router->GET("/stat", [server_ctx, &a] (http::req &req, http::resp &resp) mutable -> manapi::future<> {
                co_return resp.text(std::format("ip: {} port: {} online: {} requests: {}",
                    req.ip_data().ip,
                    req.ip_data().port,
                    server_ctx->storage().worker_data.count.load(),
                    a.load())).unwrap();
            }).unwrap();
            ...
            router->GET ("/slice", [&a] (manapi::net::http::request &req, manapi::net::http::response &resp)
                ...
                try {
                    ...
                    auto rd = (co_await manapi::fs::async_read(f.get(), sv, -1, req.cancellation().sub()));
            ...
            router->GET ("/free", [&a] (manapi::net::http::request &req, manapi::net::http::response &resp)
                ...
                co_return resp.text(std::format("requests: {}; active: {}", a.load(),
                    resp.connection_data()->worker->worker_data()->count.load())).unwrap();
            }).unwrap();
